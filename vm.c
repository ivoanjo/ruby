...
#include "ruby/vm.h"
#include "vm_core.h"
#include "hrtime.h" // TEMPORARY REPRO (not for merging)

// TEMPORARY REPRO (not for merging): widen the race window in vm_make_env_each below, gated to
// the deep closure chain built by thread-repro.rb (NEST_DEPTH=20) so ordinary shallow closures
// aren't affected. Must busy-spin, not sleep: the profiler's CPU sampler only signals on-CPU threads.
static __thread int ddtrace_repro_depth = 0;
static void ddtrace_repro_widen_race_window(void) {
    if (ddtrace_repro_depth < 19) return;
...
vm_make_env_each(const rb_execution_context_t * const ec, rb_control_frame_t *const cfp)
{
    ...
    ddtrace_repro_depth++; // TEMPORARY REPRO (not for merging)
    ...
    if (!VM_ENV_LOCAL_P(ep)) {
        ...
        if (!VM_ENV_ESCAPED_P(prev_ep)) {
            ...
            ddtrace_repro_widen_race_window(); // TEMPORARY REPRO (not for merging)
